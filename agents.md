...
- **펌웨어(`Application/`)든 연동된 Android 테스트 앱이든, 코드를 패치할 때마다 매번 `CHANGELOG.md`에 버전 항목을 추가한다.** 버전이 매우 빠르게 올라가는 프로젝트라(v0.0.1~v0.0.12가 이틀 만에 나옴), CHANGELOG가 "언제 뭘 왜 바꿨는지"를 추적하는 사실상 유일한 근거 자료다(2026-09-16 확정).
  - 기존 항목 포맷을 그대로 따른다: 굵게 강조된 소제목 + 기술적 근거/원인/검증 상태(실기 검증 완료 vs 미검증)를 명확히 구분해서 서술 (§6 자체 점검, codingstandard.md 검증 보고 구분 규칙과 일치).
  - 버그 수정은 `**[버그 수정] ...**`, 진단 결론은 `**[진단, 버그 아님으로 결론]**` 같은 기존 접두어 스타일을 유지한다.
  - 코드가 아닌 로컬 개발환경 이슈(예: 툴체인/Python 문제)도 "(참고, 코드 아님)"으로 표시해 기록한다 — 재현성에 영향을 주는 정보이므로 누락하지 않는다.
  - `config_app.h`의 `FW_VERSION_PATCH`가 이미 다음 버전으로 미리 올라가 있는 경우가 있다 — 버전 번호는 그 값을 우선 참고하고 CHANGELOG 항목과 반드시 일치시킨다.
