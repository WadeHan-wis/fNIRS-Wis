...
/* gap 식별 한계 개선(2026-09-18, m_ble_gatt.h 상단 주석 참고): 누적 ring buffer overflow
 * 카운터. read-only라 write 콜백/CCC 불필요 — VERSION(on_version_read)과 동일 패턴. */
static ssize_t on_dropped_count_read(struct bt_conn *conn, const struct bt_gatt_attr *attr,
				      void *buf, uint16_t len, uint16_t offset)
{
	...
	sys_put_le32(m_i2c_ring_buffer_get_dropped_count(), &value[0]);
 ...
 *  14: DROPPED_COUNT characteristic decl  15: DROPPED_COUNT value (read, 2026-09-18 추가)
 * m_ble_gatt_notify_data0/1/seq()가 이 인덱스에 직접 의존한다 — 배열 순서를 바꾸면 함께 수정.
 * VERSION/SEQ/DROPPED_COUNT는 맨 뒤에 추가했으므로(additive) 기존 인덱스(0~8)는 전혀
 * 변경되지 않는다.
 */
...
BT_GATT_SERVICE_DEFINE(as7341_svc,
	...
	BT_GATT_CHARACTERISTIC(BT_UUID_AS7341_DROPPED,
				BT_GATT_CHRC_READ,
				BT_GATT_PERM_READ,
				on_dropped_count_read, NULL, NULL),
);
